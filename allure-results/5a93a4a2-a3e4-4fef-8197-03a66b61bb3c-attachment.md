...
- Name: 06_Multiple_Element_\250_Multi_Element.spec.ts >> To verify Mutiple Elements in a page
- Location: tests\06_Multiple_Element_\250_Multi_Element.spec.ts:3:5
...
Call log:
  - waiting for getByText('link', { exact: true })
...
```ts
  ...
  2  | 
  3  | test('To verify Mutiple Elements in a page', async ({page})=>
  4  | {
  5  | await page.goto('https://app.thetestingacademy.com/playwright/multiple_element_filter');
  6  | const allrightpaneltext = await page.locator('a.list-group-item').allInnerTexts();
  7  | 
  8  | for (const linktext of allrightpaneltext)
  9  | {
  10 |  if(linktext === "Forgotten Password")
  11 |  {
> 12 |     await page.getByText('link', { exact: true }).click();
     |                                                   ^ Error: locator.click: Test timeout of 30000ms exceeded.
  13 |  }
  14 | }
  15 | 
