...
```
Error: locator.check: Unsupported token "@type" while parsing css selector "#preceding-sibling::td/input[@type="checkbox"]". Did you mean to CSS.escape it?
Call log:
...
```ts
  ...
  8  |      await page.locator('#employee-body').filter({hasText:'Priya.Nair'}).locator('#preceding-sibling::td/input[@type="checkbox"]'
> 9  | ).check();
     |   ^ Error: locator.check: Unsupported token "@type" while parsing css selector "#preceding-sibling::td/input[@type="checkbox"]". Did you mean to CSS.escape it?
  10 | await page.pause();
  11 |     }
