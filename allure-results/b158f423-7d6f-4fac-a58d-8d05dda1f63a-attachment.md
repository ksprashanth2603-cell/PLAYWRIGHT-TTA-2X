...
```
Error: locator.click: Unexpected token "" while parsing css selector "tr:has(td:has-text('Rohan.Mehta')". Did you mean to CSS.escape it?
Call log:
  - waiting for tr:has(td:has-text('Rohan.Mehta') >> td >> nth=0
...
```ts
  ...
  9  |     ).click();
  10 |     
> 11 |     await page.locator("tr:has(td:has-text('Rohan.Mehta')").locator("td").first().click();
     |                                                                                   ^ Error: locator.click: Unexpected token "" while parsing css selector "tr:has(td:has-text('Rohan.Mehta')". Did you mean to CSS.escape it?
  12 |     await page.pause();
  13 |     }
  14 | );
