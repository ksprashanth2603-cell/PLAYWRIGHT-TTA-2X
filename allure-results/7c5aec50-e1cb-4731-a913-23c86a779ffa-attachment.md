...
Expected pattern: /#privacy-policy/
...
Call log:
  - Expect "toHaveURL" with timeout 5000ms
    13 × locator resolved to <html lang="en" data-theme="light">…</html>
       - unexpected value "https://app.thetestingacademy.com/playwright/multiple_element_filter"
...
```ts
  ...
  9  | 
  10 | const button = await page.getByText('Privacy Policy').innerText();
  11 | //const button = await page.locator('footer a').filter({ hasText: 'Privacy Policy' }).click();
  12 | console.log(button);
  13 | //await page.pause();
> 14 | await expect(page).toHaveURL(/#privacy-policy/);
     |                    ^ Error: expect(page).toHaveURL(expected) failed
  15 | }
  16 | );
  17 | 
